...
"""creates tileset(s) from given image(s)"""

__author___ = "Georg Muntingh, Bjorn Lindeijer, Manfred Endres"
__versiom__ = "2.0"
...
class TileMap:
    ...
    def __init__(self, file, tileX, tileY):
        ...
        print("Opening the map image file: {}".format(file))
    ...
    def printProgress(self, percentage):
        ...
        """
        #print('{}}\r'.format(' '*20,))       # clean up row
    ...
    def getTileImage(self):
        ...
        TileImage = Image.new("RGBA", (self.TileX * (Xmax - Xmin + 1), self.TileY * (Ymax - Ymin + 1) ) )
        ...
        for i in range(Ymin, Ymax + 1):
            for j in range(Xmin, Xmax + 1):
                if (i,j) in H:
                    ...
                    TileImage.paste(self.TileList[H[(i,j)] - 1].convert("RGBA"), box)
    ...
    def growTileMap(self, TileMap, G, posX, posY, curV):
        ...
        cmp = lambda L1, L2: len(L1[0]) - len(L2[0])
        dir.sort(key=cmp_to_key(cmp))
        ...
        while dir != []:
            ...
            if Edges[direction] != []:
                ...
                # "belong together".
                cmp = lambda e, f: abs(e[1]) - abs(f[1])
                E.sort(key=cmp_to_key(cmp), reverse = True)
...
def list2map(tile_width, tile_heigth, *images):
    for file in images:
        map = TileMap(file, tile_width, tile_heigth)
...
def _list2map(args):
    list2map(args.tile_width, args.tile_height, *args.images)

def add_arguments_to_parser(list2map_parser, set_default_func=False):
    list2map_parser.add_argument('-tile_width', type=int, default=16)
    list2map_parser.add_argument('-tile_height', type=int, default=16)
    list2map_parser.add_argument('images', type=str, nargs='+', help='list of images to convert')

    if set_default_func:
        parser.set_defaults(func=_list2map)
